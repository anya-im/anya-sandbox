...
class AnyaDatasets(Dataset):
        ...
        for data in data_idx_list:
            ...
            for i, word in enumerate(dat["words"]):
                ...
                if i < 2:
                    ...
                    train_data["pre_id"] = self._dict.wid_bos
                else:
                    train_data["pre_id"] = dat["words"][i-2]

                train_data["cur_id"] = dat["words"][i-1]
                train_data["next_id"] = dat["words"][i]
    ...
    def __getitem__(self, idx):
        in_vec = self._dict.get_dwords(self._train_data[idx]["pre_id"], self._train_data[idx]["cur_id"])
        an_vec = self._dict.get_dwords(self._train_data[idx]["cur_id"], self._train_data[idx]["next_id"])
        return in_vec, an_vec, idx
    ...
    def __call__(self, out_model_path, inner_loop=100000):
        ...
        for i in range(self._epoch):
            ...
            for crt_t, next_t, idx in tqdm(train_loader):
                ...
                #self._dict.update_pos_vec(y[-self._dict.pos_len:], idx)
            ...
            test_crr = 0
            with torch.no_grad():
                for crt_t, next_t, idx in tqdm(test_loader):
                    x = crt_t.to(self._device)
                    next_t = next_t.to(self._device)
                    y = model(x)
